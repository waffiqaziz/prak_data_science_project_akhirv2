...
covid_gamma <- covid_lda %>% 
  tidy(matrix = "gamma") %>% 
  rename(Sentiment = document) %>% 
  arrange(Sentiment, desc(gamma))
...
```{r}
covid_gamma %>% 
  ggplot(aes(x = rev(Sentiment), y = gamma, fill = factor(topic))) +
  geom_col() +
  coord_flip()
```
...
covid_beta %>% 
  ...
  labs(
    x = "",
    y = expression(beta),
    title = "Kata Kunci Topic 1 Sentimen"
  )+
  theme_bw()
```
...
covid_beta %>% 
  ...
  labs(
    x = "",
    y = expression(beta),
    title = "Kata Kunci Topic 2 Sentimen"
  )+
  theme_bw()
```
